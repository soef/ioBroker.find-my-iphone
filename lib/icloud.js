...
ICloud.prototype.initEndpoints = function(webservices) {
    ...
    for (var i in this.functions) {
        if (!this.functions.hasOwnProperty(i)) continue;
        ...
        else if (webservices[func.endpoint]) func.path = webservices[func.endpoint].url + '/' + i;
    }
};
...
ICloud.prototype.setOwnLocation = function (location, callback) {
    ...
    if (location && typeof location === 'object') {
        ...
        this.ownLocation = this.ownLocation || { longitude: 0.0, latitude: 0.0 };
        if (location.longitude !== undefined) this.ownLocation.longitude = location.longitude;
        if (location.latitude !== undefined) this.ownLocation.latitude = location.latitude;
        if (location.str !== undefined) this.ownLocation.str = location.str;
        else this.ownLocation.str = location.latitude.toString () + ',' + location.longitude;
        return cb();
    }
    ...
    request('http://ipinfo.io', function(err, res, body) {
        if (!err && res && res.body) {
            try {
                ...
                self.setOwnLocation ( { str: json.loc } );
            } catch (e) {
            } finally {
